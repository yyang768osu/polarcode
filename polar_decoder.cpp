                      ...
                      std::vector<std::vector<double> > &P, std::vector<std::vector<std::vector<int> > > &B)
{
    if (lambda == 0)
        return;
    if (phi % 2 == 0)
        recursivelyCalcP(lambda - 1, phi / 2, m, P, B);
                      ...
                      std::vector<std::vector<std::vector<int> > > &B)
{
    ...
    recursivelyCalcB(lambda - 1, phi / 2, m, B);
}
...
std::vector<int> PolarScDecoder::polar_sc_decoder(std::vector<double> llr, std::vector<bool> info_mask)
{
